...
export function computeRateLimitReconnectDelayMs(attempt: number, err?: unknown): number {
...
export function isSpectrumRateLimited(err: unknown): boolean {
  ...
  if (e.status === 429 || e.code === 'RATE_LIMITED') return true;
  const msg = errorMessage(err);
...
export function isTransientSpectrumConnectError(err: unknown): boolean {
