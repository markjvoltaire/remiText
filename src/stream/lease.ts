...
const LEASE_HEARTBEAT_MS = Math.max(
  2_000,
  Number(process.env.WORKER_LEASE_HEARTBEAT_MS ?? Math.floor(LEASE_TTL_MS / 3)),
);
...
function isMissingLeaseSchema(err: unknown): boolean {
 ...
 */
export async function acquireLeaseOrWait(): Promise<WorkerLease> {
  const id = leaseId();
  ...
  for (;;) {
    ...
    try {
      ...
      if (isMissingLeaseSchema(err)) {
        throw new LeaseNotInstalledError(
          'worker_lease migration not applied (run supabase/migrations/20250528120000_create_worker_lease.sql)',
        );
      }
    ...
    if (ok) {
      console.log(
        `[lease] acquired id=${id} holder=${holder} ttlMs=${LEASE_TTL_MS}` +
    ...
    if (!announcedWait) {
      console.warn(
        `[lease] another worker holds id=${id} — standing by (not connecting to Spectrum to avoid 429 churn)`,
      );
      announcedWait = true;
    }
    await sleep(LEASE_WAIT_POLL_MS);
 ...
 */
export function startLeaseHeartbeat(lease: WorkerLease, onLost: () => void): () => void {
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      if (!ok && !stopped) {
        stopped = true;
        console.error(
          `[lease] lost id=${lease.id} holder=${lease.holder} — another worker took over`,
        );
        onLost();
      }
    } catch (err) {
...
export async function releaseLease(lease: WorkerLease): Promise<void> {
