 ...
 */
export async function transcribeAudio(buffer: Buffer, mimeType: string, _name: string): Promise<string> {
  ...
  const timeoutMs = Math.max(
    15_000,
    Number.parseInt(process.env.DEEPGRAM_TRANSCRIBE_TIMEOUT_MS ?? '120000', 10) || 120_000,
  );
  ...
  try {
    ...
    if (!text) throw new TranscriptionFailedError('empty transcript');
    return text;
  } catch (err) {
