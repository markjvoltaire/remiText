...
import {
  appendMessage,
  claimLocationAcknowledgment,
  getUserByPhone,
  seedLocationAcknowledgment,
} from './supabase.js';
...
function buildLocationAckMessage(location: SharedFriendLocation): string {
...
export async function handleNewLocationShare(
  contactKey: string,
  location: SharedFriendLocation,
): Promise<void> {
  if (!(await claimLocationAcknowledgment(contactKey))) return;
...
export async function seedExistingLocationSharers(): Promise<void> {
  try {
    const locations = await listSharedFriendLocations();
    for (const location of locations) {
      const contactKey = normalizeContactKey(location.address);
      if (contactKey) await seedLocationAcknowledgment(contactKey);
    }
