...
import { PoshEventCard, type PoshEventCardInput } from './templates/PoshEventCard.js';
import type { FlightCardImage } from './renderFlightCard.js';

export type { PoshEventCardInput };
...
function cacheKey(data: PoshEventCardInput): string {
  const normalized = {
    title: data.title,
    venue: data.venue,
    when: data.when,
...
async function renderToPng(data: PoshEventCardInput): Promise<Buffer> {
...
export async function generatePoshEventCardImage(
  data: PoshEventCardInput,
): Promise<FlightCardImage | null> {
