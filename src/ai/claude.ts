...
import {
  isMonidConfigured,
  runSocialDiscovery,
  MonidNotConfiguredError,
  MonidApiError,
} from '../services/monid.js';
import {
  linkAuthStatus,
  linkAuthLogin,
  linkAuthPoll,
  linkPaymentMethodsList,
  linkShippingAddressList,
  formatLinkAuthStatus,
  formatLinkLoginResponse,
  isLinkCliInstalled,
  persistLinkAuthFile,
} from '../services/linkCli.js';
import {
...
const SEARCH_PREVIEW_CARD_LIMIT = Math.max(
  0,
  Math.min(5, Number.parseInt(process.env.REMI_SEARCH_PREVIEW_CARDS ?? '0', 10) || 0),
);
...
function linkToolError(result: { error?: { message?: string }; stderr?: string }): string {
  const message =
...
async function attachFlightCardSafely(
  ...
  if (SEARCH_PREVIEW_CARD_LIMIT === 0) return;
  try {
...
async function executeTool(
  ...
  if (toolName === 'search_events') {
    ...
    const city =
      (input.city as string | undefined)?.trim() ||
      user.city?.trim() ||
      undefined;
    if (!city && !(input.keyword as string | undefined)?.trim()) {
      return JSON.stringify({
        error: true,
    ...
    try {
      const result = await searchEvents(user.id, {
        city,
        stateCode: input.state_code as string | undefined,
        keyword: input.keyword as string | undefined,
        startDateTime: input.start_date_time as string | undefined,
        endDateTime: input.end_date_time as string | undefined,
        segmentName: input.segment_name as string | undefined,
        classificationName: input.classification_name as string | undefined,
      });
      ...
      const message =
        err instanceof TicketmasterNotConfiguredError
  ...
  if (toolName === 'get_event_details') {
    ...
    try {
      ...
      const url =
        result.event && typeof result.event.url === 'string' ? result.event.url : undefined;
      return JSON.stringify({
        formatted: result.formatted,
        url,
      });
  ...
  if (toolName === 'search_tiktok' || toolName === 'search_instagram') {
    ...
    const vibe = (input.vibe as string | undefined)?.trim() || undefined;
    const tiktokKeywords =
      toolName === 'search_tiktok'
        ? ((input.keywords as string[] | undefined) ?? [])
        : ((input.tiktok_keywords as string[] | undefined) ?? []);
    const instagramHashtags =
      toolName === 'search_instagram'
        ? ((input.hashtags as string[] | undefined) ?? [])
        : ((input.instagram_hashtags as string[] | undefined) ?? []);
    ...
    if (isVagueSocialVibe(vibe)) {
      return JSON.stringify({
        needs_vibe: true,
        message:
          'Ask the user what they are in the mood for before searching — e.g. house, afrobeats, an activity, date night, romantic, brunch, clubs. Do not call this tool again until they answer with something specific.',
      });
    }
    ...
    try {
      ...
      console.log(
        `[${toolName}] location=${location} items=${payload.items.length} both_empty=${payload.both_empty}`,
      );
      return JSON.stringify(payload);
    } catch (err) {
      const message =
        err instanceof MonidNotConfiguredError
  ...
  if (toolName === 'link_auth_status') {
    if (!isLinkCliInstalled()) return linkNotAvailablePayload();

    const poll = Boolean(input.poll_until_authenticated);
    const result = poll
      ? await linkAuthPoll(user.id, (input.max_attempts as number | undefined) ?? 12)
      : await linkAuthStatus(user.id);

    if (!result.ok) return linkToolError(result);
  ...
  if (toolName === 'link_connect') {
    ...
    if (!loginResult.ok) return linkToolError(loginResult);
    ...
    return JSON.stringify({
      ...
      guidance:
        'Send the formatted message to the user. When they confirm approval, call link_auth_status with poll_until_authenticated true on the next turn.',
    });
  }
