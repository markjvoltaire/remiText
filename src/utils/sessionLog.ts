...
const USER_LOG_MAX_CHARS = Math.max(
  ...
  Math.min(
    2000,
    Number.parseInt(process.env.REMI_USER_LOG_MAX_CHARS ?? "800", 10) || 800,
  ),
);
  ...
  switch (toolName) {
    case "search_flights": {
      ...
      if (typeof obj.formatted === "string" && obj.formatted.length > 0) {
        ...
        lines.push(
          `formatted: ${preview}${obj.formatted.includes("\n") ? " …" : ""}`,
        );
      }
      break;
    }
    case "search_restaurants": {
      ...
      if (typeof obj.formatted === "string" && obj.formatted.length > 0) {
        ...
        lines.push(
          `formatted: ${firstLine}${obj.formatted.includes("\n") ? " …" : ""}`,
        );
      }
      break;
    }
    ...
    case "search_instagram": {
      ...
      lines.push(
        `${items.length} item(s), both_empty=${String(obj.both_empty ?? false)}`,
      );
...
export function sessionAssistantLog(
  text: string,
  meta?: { attachments?: number },
): void {
  if (!enabled()) return;
