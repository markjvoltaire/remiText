...
export function mealPeriodWindow(period) {
...
export function parseMealPeriodFromText(text) {
    const t = text.toLowerCase();
    if (/\b(late night|after midnight)\b/.test(t))
        return 'late_night';
    ...
    if (/\b(night|dinner|evening|supper)\b/.test(t))
        return 'night';
    if (/\b(brunch)\b/.test(t))
        return 'brunch';
    if (/\b(lunch|midday|noon)\b/.test(t))
        return 'lunch';
    if (/\b(breakfast|morning)\b/.test(t))
        return 'breakfast';
    if (/\b(afternoon)\b/.test(t))
        return 'afternoon';
    return null;
}
...
export function slotTimeToMinutes(timeStr) {
    ...
    if (compact) {
        ...
        if (compact[3].toLowerCase() === 'p' && hour !== 12)
            hour += 12;
        if (compact[3].toLowerCase() === 'a' && hour === 12)
            hour = 0;
        return hour * 60 + minute;
    }
    return null;
}
export function slotMatchesMealPeriod(timeStr, period) {
...
export function filterSlotsByMealPeriod(slots, period) {
    return slots.filter((slot) => slotMatchesMealPeriod(slot.time, period));
}
export function filterVenuesByMealPeriod(venues, period) {
    return venues
        .map((venue) => ({
        ...venue,
...
function uniqueSlotTimes(slots) {
    const seen = new Set();
    const out = [];
...
export function pickCuratedSlotTimes(slots, limit = 3, mealPeriod) {
...
export function formatCuratedTimesPhrase(slots, limit = 3, mealPeriod) {
