...
const TOOL_LOG_MAX_CHARS = Math.max(400, Math.min(20_000, Number.parseInt(process.env.REMI_TOOL_LOG_MAX_CHARS ?? "6000", 10) || 6000));
const USER_LOG_MAX_CHARS = Math.max(80, Math.min(2000, Number.parseInt(process.env.REMI_USER_LOG_MAX_CHARS ?? "800", 10) || 800));
...
function formatValue(value, maxChars) {
...
function summarizeToolResult(toolName, result, parsed) {
    const lines = [];
    ...
    if (obj.error === true) {
        ...
        if (obj.instant_only)
            lines.push("instant payment required (hold not available)");
        if (obj.stale_offer)
            lines.push("stale offer — fresh search included in result");
        if (obj.needs_vibe)
            lines.push("needs user vibe before social search");
        return lines;
    }
    switch (toolName) {
        case "search_flights": {
        ...
        case "confirm_booking": {
            ...
            if (obj.order_id)
                lines.push(`order_id: ${String(obj.order_id)}`);
            if (obj.offer_id)
                lines.push(`offer_id: ${String(obj.offer_id)}`);
            break;
        }
        case "search_tiktok":
        ...
        case "cancel_restaurant_reservation": {
            ...
            if (Array.isArray(obj.slots))
                lines.push(`${obj.slots.length} slot(s)`);
...
export function sessionTurnStart(opts) {
    ...
    console.log(BANNER);
    const userBody = opts.agentInput && opts.agentInput !== opts.inboundText
...
export function sessionModelRound(opts) {
    if (!enabled() || !activeTurn)
        return;
    activeTurn.modelRound = opts.round;
    const tools = opts.toolNames && opts.toolNames.length > 0
        ? ` → ${opts.toolNames.join(", ")}`
        : "";
    console.log(`  MODEL  round=${opts.round}  stop=${opts.stopReason}${tools}`);
}
export function sessionToolLog(toolName, input, result, meta) {
    if (!enabled() || !activeTurn)
        return;
    ...
    const body = parsed !== null
        ? formatValue(parsed, TOOL_LOG_MAX_CHARS)
        : formatValue(result, TOOL_LOG_MAX_CHARS);
    console.log(indentBlock(body));
}
export function sessionAssistantLog(text, meta) {
    if (!enabled())
        return;
    const { text: body } = truncate(text, USER_LOG_MAX_CHARS * 2);
    const attach = meta?.attachments !== undefined
        ? `  (${text.length} chars, ${meta.attachments} attachment(s))`
        : "";
    console.log(LINE);
    console.log(`  ASSISTANT → user${attach}`);
    console.log(indentBlock(body));
    if (activeTurn) {
        console.log(`  TURN DONE  tools=${activeTurn.toolCount}  model_rounds=${activeTurn.modelRound || "—"}`);
        console.log(BANNER);
        console.log("");
        activeTurn = null;
    }
}
export function sessionTurnAbort(reason) {
    if (!enabled())
        return;
