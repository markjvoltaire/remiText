...
export function sessionModelRound(opts) {
    ...
    const tools = opts.toolNames && opts.toolNames.length > 0 ? ` → ${opts.toolNames.join(', ')}` : '';
...
export function sessionAssistantLog(text, meta) {
    ...
    const attach = meta?.attachments !== undefined ? `  (${text.length} chars, ${meta.attachments} attachment(s))` : '';
    console.log(LINE);
