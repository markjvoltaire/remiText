...
export function isResyConfigured() {
    ...
    const hasLogin = Boolean(process.env.RESY_EMAIL?.trim()) &&
...
export async function warmResyAuth() {
    if (!isResyConfigured())
        return;
    authToken = '';
...
async function resyPostJson(path, body) {
    await ensureAuth();
...
async function resyPostForm(path, fields) {
    await ensureAuth();
    const url = `${RESY_BASE_URL}${path}`;
    for (let attempt = 0; attempt < 3; attempt += 1) {
        if (attempt > 0)
...
function pickPaymentMethodId(methods, override) {
    if (override != null)
        return override;
    ...
    if (id == null) {
        throw new ResyAuthError("No Resy payment method on file. Add a card in Resy or set RESY_PAYMENT_METHOD_ID.");
    }
    return id;
}
/** Step 1 of booking: exchange config token (from /4/find) for a short-lived book token. */
export async function getBookingDetails(configToken, date, partySize) {
    if (!isResyConfigured())
        throw new ResyNotConfiguredError();
...
export async function bookReservation(bookToken, paymentMethodId) {
    if (!isResyConfigured())
        throw new ResyNotConfiguredError();
...
export async function listUpcomingReservations() {
    if (!isResyConfigured())
        throw new ResyNotConfiguredError();
    const data = (await resyGet("/3/user/reservations"));
    ...
    for (const r of raw) {
        ...
        out.push({
            ...
            type: String(r.config?.type ?? r.type ?? "") || undefined,
        });
    }
    return out;
}
/** Cancel a reservation by resy_token (POST /3/cancel, form-encoded). */
export async function cancelReservation(resyToken) {
    if (!isResyConfigured())
        throw new ResyNotConfiguredError();
...
export async function reserveRestaurantTable(params) {
    const details = await getBookingDetails(params.configToken, params.date, params.partySize);
    const booked = await bookReservation(details.bookToken, params.paymentMethodId ?? details.paymentMethodId);
    return {
        ...booked,
...
export function normalizeReservationTimeLabel(input) {
...
export function slotMatchesUserTime(slotTime, userTime) {
    const slot = slotTime.trim();
    const raw = userTime.trim();
    if (!slot || !raw)
        return false;
    if (slot.toLowerCase() === raw.toLowerCase())
        return true;
    const normalized = normalizeReservationTimeLabel(raw);
    if (normalized && slot === normalized)
        return true;
    return slot.toLowerCase().startsWith(raw.toLowerCase());
}
export function findVenueSlotByTime(venue, time) {
